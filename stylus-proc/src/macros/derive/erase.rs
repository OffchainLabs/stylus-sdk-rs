...
fn impl_erase(node: &syn::ItemStruct) -> syn::ItemImpl {
    ...
    parse_quote! {
        impl #impl_generics stylus_sdk::storage::Erase<H> for #name #ty_generics #where_clause {
            fn erase(&mut self) {
                #(
    ...
    fn test_impl_erase() {
        assert_ast_eq(
            impl_erase(&parse_quote! {
                struct Foo<H: stylus_sdk::host::Host, T: Erase> {
                    field1: StorageString<H>,
                    field2: T,
                }
            }),
            parse_quote! {
                impl<H: stylus_sdk::host::Host, T: Erase> stylus_sdk::storage::Erase<H> for Foo<H, T> {
                    fn erase(&mut self) {
                        self.field1.erase();
                        self.field2.erase();
                    }
